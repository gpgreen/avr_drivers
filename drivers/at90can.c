...
struct can_device at90can_dev = {
  .priv_dev = &s_at90can_dev_priv,
  .settings = 0,
  .devno = 1,
  .bus_state = ERROR_ACTIVE,
  .init_fn = at90can_init,
  .reinit_fn = at90can_reinit,
  ...
  .device_command = can_device_command,
...
ISR(CANIT_vect)
{
  ...
  for (int i=0; i<15; ++i) {
	...
	if (s_at90can_dev_priv.dev->settings->filters.filtering_on) {
	  CANIDT1 = (uint8_t)(s_at90can_dev_priv.dev->settings
			      ->filters.standard_id_filter[i] >> 3);
	  CANIDT2 = (uint8_t)(s_at90can_dev_priv.dev->settings
...
static void at90can_install_filters(struct can_device* dev)
{
 ...
 */
int at90can_init(can_init_t* settings, struct can_device* dev)
{
  // set the dev on the priv structure
  s_at90can_dev_priv.dev = dev;
  
  /* set the global variable */
  dev->settings = settings;

  return at90can_reinit(dev);
}

/*
 * Re-initialize the AT90CAN controller
 * returns: CAN_OK, or CAN_FAILINIT
 */
int at90can_reinit(struct can_device* dev)
{
  ...
  switch (dev->settings->speed_setting) {
  case (CAN_20KBPS):
#ifdef AT90CANDEBUG
    DEVICE_PRINT("20kbps");
  ...
  at90can_install_filters(dev);

  // put the controller in listening mode, if loopback is requested
  if (dev->settings->loopback_on) {
#ifdef AT90CANDEBUG
    DEVICE_PRINT2("setting loopback mode:%x", dev->settings->loopback_on);
#endif   
    CANGCON |= _BV(LISTEN);
  }
 ...
 */
int at90can_self_test(struct can_device* dev)
{
  // we are enabled
  ...
  if (dev->settings->loopback_on && !bit_is_set(CANGCON, LISTEN))
    return CAN_FAIL;
  return CAN_OK;
}
...
int at90can_get_next_free_tx_mob(struct can_device* dev, int* txbuf_n)
{
  int i;
  *txbuf_n = -1;
  struct at90can_priv* pdev = (struct at90can_priv*)dev->priv_dev;
  for (i=pdev->first_tx_mob; i<15; ++i) {
 ...
 *
 */
void at90can_write_msg(struct can_device* dev,
		       int tx_mob_idx,
 ...
 */
int at90can_check_msg_received(struct can_device* dev)
{
 ...
 * read a CAN message
 */
int at90can_read_msg(struct can_device* dev, can_msg_t *p_message)
{
  uint8_t recv_buf_count;
  ATOMIC_BLOCK(ATOMIC_FORCEON)
  {
 ...
 */
int at90can_handle_interrupt(struct can_device* dev, int* status_flag)
{
  struct at90can_priv* pdev = (struct at90can_priv*)dev->priv_dev;
  ATOMIC_BLOCK(ATOMIC_FORCEON) 
  {
    *status_flag = pdev->flag;
    pdev->flag = 0;
  }
  if (*status_flag) {
...
// returns CAN_OK, CAN_FAIL
int at90can_error_counts(struct can_device* dev,
...
void at90can_clear_tx_buffers(struct can_device* dev)
{
